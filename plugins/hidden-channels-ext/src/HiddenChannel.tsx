...
export default function HiddenChannel({channel}) {
    return <View style={MessageStyles.container}>
        <Text style={MessageStyles.title}>
        ...
        <Text style={MessageStyles.text}>Creation Date: </Text>
        <FancyDate date={new Date(snowflakeUtils.extractTimestamp(channel.id))} />
        <Text>
            {channel.lastMessageId ? <FancyDate date={new Date(snowflakeUtils.extractTimestamp(channel.lastMessageId))} /> : "No messages."}
            {"\n"}
            {channel.lastPinTimestamp ? <FancyDate date={new Date(channel.lastPinTimestamp)} /> : "No pins."}
        </Text>
    </View>
}
